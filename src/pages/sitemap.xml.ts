...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Burton upon Trent gets higher priority)
    const isBurton = location.slug === 'burton-upon-trent';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isBurton ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isBurton ? 1 : 2 + i),
    });
